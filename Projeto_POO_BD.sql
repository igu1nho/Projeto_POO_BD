...
CREATE TABLE IF NOT EXISTS `Projeto_POO_BD`.` Status_jog` (
  `id Status_jog` INT AUTO_INCREMENT NOT NULL,
  `kills` INT NOT NULL,
  `deaths` INT NOT NULL,
  `horas_p_dia` VARCHAR(5) NULL,
  PRIMARY KEY (`id Status_jog`));
...
CREATE TABLE IF NOT EXISTS `Projeto_POO_BD`.`Jogador` (
  ...
  ` Status_jog_id Status_jog` INT NOT NULL,
  PRIMARY KEY (`idJogador`),
  INDEX `fk_Jogador_Org_idx` (`Org_idOrg` ASC) VISIBLE,
  INDEX `fk_Jogador_ Status_jog1_idx` (` Status_jog_id Status_jog` ASC) VISIBLE,
  CONSTRAINT `fk_Jogador_Org`
    FOREIGN KEY (`Org_idOrg`)
    REFERENCES `Projeto_POO_BD`.`Org` (`idOrg`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Jogador_ Status_jog1`
    FOREIGN KEY (` Status_jog_id Status_jog`)
    REFERENCES `Projeto_POO_BD`.` Status_jog` (`id Status_jog`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
